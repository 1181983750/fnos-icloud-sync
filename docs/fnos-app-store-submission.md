...
- Release 地址：https://github.com/1181983750/fnos-icloud-sync/releases/tag/v0.3.0
- FPK 下载地址：https://github.com/1181983750/fnos-icloud-sync/releases/download/v0.3.0/icloud-sync-0.3.0.fpk
- FPK SHA256：D7C414D4853ACB0C5DC2D510ABD62C799FD0C393744259A8E169F9CBD26EE55C
...
- 启动引导：桌面入口先进入光效视差启动页，等待 Python 虚拟环境、依赖和 Web 服务就绪后进入同步面板。
- 启动引导面向普通用户展示“正在准备同步服务”，默认隐藏端口、候选入口和依赖等调试信息。
...
1. 在 fnOS 应用中心手动安装 `icloud-sync-0.3.0.fpk`。
2. 启动应用，确认桌面入口可打开光效视差启动页。
3. 等待原生服务启动完成，进入 iCloud 同步面板。
...
0.3.0 应用改为原生 FPK 运行方式，不再注册 Docker Compose 项目；启动时在应用数据目录创建 Python 虚拟环境并优先使用内置 wheels 安装依赖，服务直接读取应用设置中的同步根目录，停止并重新启动应用后即可切换写入位置。
