...
- 本地持久化：配置、Cookie、日志存在应用数据目录；下载结果写入同步根目录，容器内固定为 `/data`，飞牛会把你在应用设置中选择的 NAS 目录挂载到 `/data`。
...
Web 面板会同时显示“目标保存路径”和“当前容器实际写入路径”。如果提示当前容器仍挂载在旧目录，说明飞牛还没有重建容器挂载，需要在应用中心停止后重新启动应用。
...
1. 在飞牛应用中心手动安装 `.fpk`。
2. 启动应用，首次启动会先显示光效视差启动页，并等待 Docker 容器、Web 服务和依赖准备完成。
3. 如果想把文件存到任意位置，先进入飞牛“应用设置”，为本应用授权目录，并在配置向导里选择同步根目录；保存后必须停止并重新启动应用，让容器把新目录挂载到 `/data`。
4. 打开桌面入口「iCloud 同步」。
