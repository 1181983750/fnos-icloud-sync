...
RUNTIME_ENV_FILE="${STORAGE_DIR}/runtime.env"
ACCESSIBLE_PATHS=""
...
write_runtime_env() {
    ...
    {
        printf '# Generated by fnOS iCloud Sync app settings. cmd/main reads storage_root.json and keeps this file for diagnostics.\n'
        printf "SYNC_ROOT_HOST_PATH='%s'\n" "$(dotenv_escape "$selected_root")"
        printf "APP_DATA_DIR='%s'\n" "$(dotenv_escape "$selected_root")"
        if [ -n "${TRIM_PKGVAR:-}" ]; then
            printf "APP_CONFIG_DIR='%s'\n" "$(dotenv_escape "${TRIM_PKGVAR}/config")"
            printf "APP_LOG_DIR='%s'\n" "$(dotenv_escape "${TRIM_PKGVAR}/logs")"
        fi
        if [ -n "${TRIM_SERVICE_PORT:-}" ]; then
            printf "APP_PORT='%s'\n" "$(dotenv_escape "$TRIM_SERVICE_PORT")"
        fi
    } > "${RUNTIME_ENV_FILE}" 2>/dev/null || {
        log_config_message "failed to write runtime env: ${RUNTIME_ENV_FILE}"
        return 0
    }

    log_config_message "wrote runtime env: ${RUNTIME_ENV_FILE}"
}
...
write_wizard_config() {
    ...
    if [ "$path_count" -gt 0 ]; then
        help_text="你可以把同步根目录切换到飞牛里已授权的任意目录。保存后请重启应用，让服务读取新的目录。"
...
EOF
        ...
        printf '      {\n'
        printf '        "type": "tips",\n'
        printf '        "helpText": "%s"\n' "$(json_escape "详细同步选项请打开桌面图标进入 Web 面板设置。若修改了同步根目录，请保存后重启应用，让服务读取新的目录。")"
        printf '      }\n'
        printf '    ]\n'
        printf '  }\n'
        printf ']\n'
